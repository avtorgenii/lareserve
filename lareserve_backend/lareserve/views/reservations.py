...
def reservation_list_today(request):
    ...
    reservations = Reservation.objects.filter(
        restaurant_id=restaurant_id, date__date=today
    ).order_by("date")
...
def reservation_by_table(request):
    ...
    if not all([restaurant_id, table_id, date_str]):
        return Response(
            {"error": "restaurant_id, table_id and date are required"},
            status=status.HTTP_400_BAD_REQUEST,
        )

    reservations = Reservation.objects.filter(
        restaurant_id=restaurant_id, table_id=table_id, date__date=date_str
    ).order_by("date")
...
@extend_schema(
    request=ReservationStatusUpdateSerializer, responses={200: ReservationSerializer}
)
@api_view(["PUT"])
@permission_classes([IsAuthenticated])
